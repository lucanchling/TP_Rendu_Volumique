...
image mip(volume const& vol)
{
    ...
    for (int y=0;y<vol.size_y();y++)
    {
        for (int z=0; z<vol.size_z();z++)
        {
            float intensite_max = 0;
            for (int x=0; x<vol.size_x();x++)
            {
                if (vol(x,y,z)>intensite_max)
                    intensite_max = vol(x,y,z);
            }
            im(y,z) = intensite_max;
        }
    }
